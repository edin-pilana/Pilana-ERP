...
export default function PilanaPeriodIzvjestaj({ data, datumOd, datumDo, lokacija, brentistaFilter }) {
      ...
      <ul className="text-[8px] flex flex-wrap justify-center gap-1.5 mt-2 leading-tight">
        {payload.map((entry, index) => (
          <li key={`item-${index}`} className="flex items-center font-bold">
            <span className="w-2 h-2 rounded-full inline-block mr-1" style={{ backgroundColor: entry.color, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}></span>
  ...
  return (
    <>
      {/* MAGIJA: CSS koji izoluje samo ovaj izvještaj prilikom printanja! */}
      <style>{`
        @media print {
          body * { visibility: hidden; }
          .print-izolacija, .print-izolacija * { visibility: visible; }
          .print-izolacija { 
      ...
      <div className="print-izolacija bg-white text-black p-6 mx-auto max-w-[210mm] min-h-[297mm] shadow-2xl relative font-sans flex flex-col justify-between" style={{ WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-6 mb-6">
            {/* ULAZ */}
            <div>
              <h3 className="font-bold text-white text-[11px] p-1.5 px-2 mb-2 rounded-t" style={{ backgroundColor: tamnoPlava, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>STRUKTURA ULAZA (SIROVINA)</h3>
            ...
            {/* IZLAZ */}
            <div>
              <h3 className="font-bold text-white text-[11px] p-1.5 px-2 mb-2 rounded-t" style={{ backgroundColor: tamnoPlava, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>STRUKTURA IZLAZA (PROIZVODI)</h3>
              <table className="w-full text-[10px] text-left border-collapse">
          ...
          <h3 className="font-bold text-white text-[11px] p-1.5 px-2 mb-2 rounded-t" style={{ backgroundColor: tamnoPlava, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>RANG LISTA BRENTISTA (UČINAK U PERIODU)</h3>
            ...
            {leaderboard.map((radnik, idx) => (
              <div key={idx} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg border-l-4" style={{ borderColor: idx === 0 ? limetaZelena : tamnoPlava }}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[12px] font-black uppercase text-gray-800">{radnik.ime} {idx === 0 && <span className="text-[8px] text-white px-1.5 py-0.5 rounded ml-1" style={{ backgroundColor: '#f59e0b', WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>TOP 1</span>}</p>
          ...
          <h3 className="font-bold text-white text-[11px] p-1.5 px-2 mb-3 rounded-t print:mt-4" style={{ backgroundColor: tamnoPlava, WebkitPrintColorAdjust: 'exact', printColorAdjust: 'exact' }}>VIZUELNA ANALIZA PERIODA</h3>
        ...
        {/* Print dugme - automatski se skriva pri printanju */}
