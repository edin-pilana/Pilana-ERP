...
export default function Page() {
    ...
    const spasiDizajn = async () => {
        ...
        if(error) {
            toast.error("Greška pri snimanju dizajna: " + error.message);
        } else { 
            toast.success("Dizajn uspješno spašen!"); 
            setIsEditMode(false); 
            setBackupPostavke(null); 
        }
    };
    ...
    const handleLogin = async (e) => {
        ...
        if (error) {
            ...
            setLoggedUser(data);
            toast.success(`Dobrodošli nazad, ${data.ime_prezime}!`);
        } else {
            toast.error("Pogrešan Username ili Password!");
        }
    };
                        ...
                        <div className="max-w-5xl mx-auto space-y-8">
                            ...
                            <div className={`border border-theme-border p-8 rounded-[var(--radius-box)] flex flex-col md:flex-row gap-4 justify-between items-center shadow-2xl transition-colors duration-500 bg-theme-card backdrop-blur-[var(--glass-blur)] ${isEditMode ? 'ring-4 ring-amber-500' : ''}`}>
                                <div className="w-full md:w-auto">
                                    {isEditMode ? (
                                        ...
                                        <>
                                            ...
                                            <h1 className="text-4xl text-theme-text font-black">Zdravo, {loggedUser?.ime_prezime || 'Korisnik'}</h1>
                                ...
                                <div className="flex gap-3 items-center w-full md:w-auto justify-end">
                                    ...
                                    {!isEditMode && (
                                        <button onClick={() => { localStorage.removeItem('smart_timber_user'); window.location.reload(); }} className="px-6 py-3 rounded-2xl bg-theme-panel text-theme-muted font-bold text-xs uppercase border border-theme-border hover:bg-red-500 hover:text-white transition-all shadow-lg">Odjava</button>
                                    )}
                                </div>
                            </div>
                                        ...
                                        >
                                            {isEditMode ? (
                                                ...
                                                <div 
                                                    ...
                                                    className="p-6 rounded-[var(--radius-box)] flex items-center justify-center cursor-pointer transition-all hover:scale-105 shadow-[0_15px_35px_rgba(0,0,0,0.5)] h-32 relative overflow-hidden group border border-white/10"
