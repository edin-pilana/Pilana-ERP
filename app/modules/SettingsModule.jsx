...
function SettingsSearchable({ label, value, onChange, list = [], placeholder="..." }) {
    ...
    return (
        <div className="relative font-black">
            ...
            <input type="text" value={value} onFocus={() => setOpen(true)} onChange={e => { onChange(e.target.value.toUpperCase()); setOpen(true); }} className="w-full p-3 bg-theme-panel border border-theme-border rounded-xl text-xs text-theme-text outline-none focus:border-blue-500 uppercase" placeholder={placeholder} />
            {open && list.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-theme-panel border border-theme-border rounded-xl shadow-2xl max-h-40 overflow-y-auto">
                    {list.filter(i => i.toUpperCase().includes(value.toUpperCase())).map((item, idx) => (
                        <div key={idx} onClick={() => { onChange(item); setOpen(false); }} className="p-3 text-[10px] border-b border-theme-border hover:bg-theme-accent uppercase cursor-pointer text-theme-text">{item}</div>
                    ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in">
            <div className="space-y-4">
                ...
                <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                    ...
                    <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                        {sumarije.map(s => (
                            <div key={s.id} onClick={() => pokreniIzmjenuSum(s)} className="flex justify-between items-center p-3 bg-theme-card border border-theme-border rounded-xl cursor-pointer hover:border-emerald-500/50 transition-all">
            ...
            <div className="space-y-4">
                ...
                <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                    ...
                    <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                        {podruznice.map(p => (
                            <div key={p.id} onClick={() => pokreniIzmjenuPodr(p)} className="flex justify-between items-center p-3 bg-theme-card border border-theme-border rounded-xl cursor-pointer hover:border-emerald-500/50 transition-all">
...
function TabPrevoznici() {
    ...
    return (
        <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl max-w-2xl mx-auto animate-in slide-in-from-right space-y-6">
            <h3 className="text-amber-400 font-black uppercase text-xs mb-4">🚚 Upravljanje Prevoznicima</h3>

            <div className="flex gap-2">
                <input value={noviPrevoznik} onChange={e=>setNoviPrevoznik(e.target.value)} placeholder="Novi prevoznik (npr. TRANS-KOP)" className="flex-1 p-4 bg-theme-card border border-theme-border rounded-xl text-xs text-theme-text outline-none focus:border-amber-500 uppercase font-bold" />
                <button onClick={dodajPrevoznika} className="px-8 bg-amber-600 text-theme-text font-black rounded-xl hover:bg-amber-500 shadow-lg uppercase text-[10px]">➕ Dodaj</button>
            </div>

            <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                {prevoznici.map(p => (
                    <div key={p.id} className="flex justify-between items-center p-4 bg-theme-card border border-theme-border rounded-xl group hover:border-amber-500/30 transition-all">
                        <span className="text-sm text-theme-text font-bold uppercase">{p.naziv}</span>
...
function TabMasine() {
    ...
    return (
        <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-2xl space-y-6 animate-in fade-in">
            <div className={`space-y-4 p-5 rounded-box border transition-all ${isEditing ? 'border-amber-500/50 bg-theme-panel' : 'border-theme-border bg-theme-card'}`}>
                <div className="flex justify-between items-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="col-span-1 md:col-span-3 mt-2 bg-theme-panel p-4 rounded-xl border border-theme-border">
                        ...
                        <div className="flex gap-6">
                            {['prorez', 'pilana', 'dorada'].map(modul => (
                                <label key={modul} className="flex items-center gap-2 text-theme-text text-xs font-bold cursor-pointer uppercase hover:text-theme-accent transition-colors">
                                    <input type="checkbox" checked={form.dozvoljeni_moduli?.includes(modul) || false} onChange={(e) => { const trenutni = form.dozvoljeni_moduli || []; setForm({ ...form, dozvoljeni_moduli: e.target.checked ? [...trenutni, modul] : trenutni.filter(m => m !== modul) }); }} className="w-5 h-5 accent-blue-600 bg-theme-panel border-theme-border rounded" />
                                    {modul}
                                </label>
                            ))}
                        </div>
                    </div>
                    <div className="col-span-1 md:col-span-3 pt-4 border-t border-theme-border mt-2">
                            ...
                            {masterOznake.map(oznaka => {
                                ...
                                return (
                                    <div key={oznaka} className="flex items-center">
                                        <button onClick={() => toggleOznakaMasine(oznaka)} className={`px-3 py-2 rounded-l-xl text-[10px] font-black uppercase transition-all border-y border-l ${aktivan ? 'bg-emerald-600 border-emerald-400 text-theme-text shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-theme-panel border-theme-border text-slate-500 hover:border-slate-500'}`}>{aktivan ? '✓ ' : '+ '} {oznaka}</button>
                                        <button onClick={() => obrisiIzMastera(oznaka)} className={`px-2 py-2 border-y border-r rounded-r-xl text-[10px] font-black transition-all ${aktivan ? 'bg-emerald-700 border-emerald-400 text-emerald-300 hover:text-theme-text hover:bg-red-500' : 'bg-theme-card border-theme-border text-slate-600 hover:text-red-500'}`} title="Trajno obriši iz sistema">✕</button>
                                    </div>
                                );
                            })}
                        </div>
                        <div className="flex gap-2 max-w-sm mt-4 bg-theme-panel p-2 rounded-xl border border-theme-border">
                            <input value={novaMasterOznaka} onChange={e=>setNovaMasterOznaka(e.target.value)} onKeyDown={e=>{if(e.key==='Enter') dodajUMaster()}} placeholder="Nova operacija u sistemu..." className="flex-1 p-2 bg-transparent text-xs text-theme-text outline-none uppercase" />
                            <button onClick={dodajUMaster} className="px-4 bg-theme-accent rounded-lg text-theme-text font-black hover:opacity-80 text-[10px] uppercase shadow-lg">+ Baza</button>
                        </div>
                    </div>
                </div>
                <button onClick={save} className={`w-full py-4 text-theme-text font-black rounded-xl text-xs shadow-lg uppercase transition-all mt-4 ${isEditing ? 'bg-amber-600 hover:bg-amber-500' : 'bg-theme-accent hover:opacity-80'}`}>{isEditing ? '✅ Ažuriraj Mašinu' : '➕ Dodaj Mašinu'}</button>
            </div>
            ...
            <div className="space-y-2 pt-2">
                ...
                <div className="max-h-80 overflow-y-auto pr-2 space-y-3">
                    ...
                    {masine.map(m => (
                        <div key={m.id} onClick={() => pokreniIzmjenu(m)} className={`flex flex-col p-4 border rounded-2xl cursor-pointer transition-all ${isEditing && form.id === m.id ? 'bg-theme-panel border-amber-500/50' : 'bg-theme-card border-theme-border hover:border-blue-500/50'}`}>
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="font-black text-theme-text text-sm">{m.naziv}</p>
                                    <p className="text-[10px] text-slate-400 mt-1">Radni sat: <b className="text-emerald-400">{m.cijena_sat} KM</b> | Po kubiku: <b className="text-emerald-400">{m.cijena_m3} KM</b> | Moduli: <b className="text-theme-accent">{m.dozvoljeni_moduli ? m.dozvoljeni_moduli.toUpperCase() : 'SVI'}</b></p>
                                </div>
                                <button onClick={(e)=>{e.stopPropagation(); obrisi(m.id, m.naziv);}} className="text-red-500 font-black px-4 py-2 bg-red-900/20 rounded-xl hover:bg-red-500 hover:text-theme-text transition-all">✕</button>
                            </div>
                            <div className="flex flex-wrap gap-1 mt-3">
                                {(m.atributi_paketa || []).length === 0 ? (
                                    <span className="text-[8px] text-slate-500 font-bold italic border border-theme-border px-2 py-0.5 rounded">Nema dodijeljenih operacija</span>
                                ) : (
    ...
    return (
        <div className="space-y-4 animate-in fade-in">
            ...
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-amber-500/30 shadow-2xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end border-b border-theme-border pb-4 mb-4">
                    <div>
                        <label className="text-[10px] text-slate-500 uppercase ml-2 block mb-1">1. Odaberi Kategoriju</label>
                        <select value={odabranaKategorija} onChange={e=>setOdabranaKategorija(e.target.value)} className="w-full p-4 bg-theme-card rounded-xl text-sm text-theme-text border border-theme-border outline-none focus:border-amber-500 uppercase font-black cursor-pointer">
                    ...
                    {odabranaKategorija && (
                        <div className="flex gap-2 items-center bg-theme-card p-2 rounded-xl border border-theme-border">
                            ...
                            <button onClick={primijeniNaSveIspod} className="bg-theme-panel text-amber-500 hover:bg-amber-600 hover:text-theme-text px-4 py-3 rounded-lg text-[10px] font-black uppercase transition-all shadow-md">👇 Primijeni ispod</button>
                        </div>
                    )}
                </div>
                    ...
                    <div className="animate-in slide-in-from-top-4">
                        <div className="max-h-60 overflow-y-auto pr-2 space-y-2 mb-4">
                            {proizvodiUKategoriji.map(p => (
                                <div key={p.sifra} className="flex justify-between items-center bg-theme-card p-3 rounded-xl border border-theme-border hover:border-slate-600 transition-all">
                                    <div className="flex-1">
                                        <p className="text-theme-text text-xs font-black">{p.sifra}</p>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="relative">
                                            ...
                                            <input 
                                                ...
                                                className="w-32 pl-12 pr-3 py-2 bg-theme-panel border border-amber-500/50 rounded-lg text-amber-400 font-black outline-none focus:border-amber-400"
                                            />
                                        </div>
                                    </div>
                                </div>
                        ...
                        <button onClick={snimiNoveCijeneBaza} className="w-full py-4 bg-amber-600 text-theme-text px-6 rounded-xl font-black text-xs uppercase shadow-lg hover:bg-amber-500 transition-all">
            ...
            <div className={`bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border shadow-2xl space-y-4 transition-all ${isEditing ? 'border-amber-500/50' : 'border-theme-border'}`}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="col-span-2">
                        ...
                        <input value={form.sifra} disabled={isEditing} onChange={e=>setForm({...form, sifra: e.target.value.toUpperCase()})} className={`w-full p-3 rounded-xl text-xs text-theme-text uppercase font-black outline-none ${isEditing ? 'bg-theme-panel border border-theme-border opacity-50 cursor-not-allowed' : 'bg-theme-panel border border-blue-500/50 focus:border-blue-400'}`} />
                    </div>
                    <div className="col-span-2">
                        <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">* NAZIV</label>
                        <input value={form.naziv} onChange={e=>setForm({...form, naziv:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-blue-500" />
                    </div>
                    
                    <div className="relative z-40 col-span-2">
                        <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Kategorija</label>
                        <input value={form.kategorija} onChange={e=>setForm({...form, kategorija:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-blue-500" placeholder="..." />
                    </div>
                    <div className="col-span-2">
                        <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Dimenzije tekst</label>
                        <input value={form.dimenzije} onChange={e=>setForm({...form, dimenzije:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-blue-500" />
                    </div>

                    <div>
                        <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Def. Jedinica</label>
                        <select value={form.default_jedinica} onChange={e=>setForm({...form, default_jedinica:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-blue-500">
                    ...
                    <div>
                        ...
                        <input type="number" value={form.duzina} onChange={e=>setForm({...form, duzina:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-center text-amber-400 border border-theme-border outline-none focus:border-blue-500" />
                    </div>
                    <div>
                        <label className="text-[8px] text-amber-500 uppercase ml-2 block mb-1">Širina (cm)</label>
                        <input type="number" value={form.sirina} onChange={e=>setForm({...form, sirina:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-center text-amber-400 border border-theme-border outline-none focus:border-blue-500" />
                    </div>
                    <div>
                        <label className="text-[8px] text-amber-500 uppercase ml-2 block mb-1">Visina/Deb (cm)</label>
                        <input type="number" value={form.visina} onChange={e=>setForm({...form, visina:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-center text-amber-400 border border-theme-border outline-none focus:border-blue-500" />
                    </div>
                </div>
                <button onClick={snimiProizvod} className={`w-full py-4 text-theme-text font-black rounded-xl text-xs shadow-lg uppercase transition-all ${isEditing ? 'bg-amber-600 hover:bg-amber-500' : 'bg-theme-accent hover:opacity-80'}`}>
            ...
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                ...
                <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2">
                    {katalog.map(k => (
                        <div key={k.sifra} onClick={() => pokreniIzmjenu(k)} className="flex justify-between items-center p-3 bg-theme-card border border-theme-border rounded-xl cursor-pointer hover:border-blue-500/50 transition-all">
                            <div>
                                <p className="text-theme-text text-xs font-black">{k.sifra} <span className="text-theme-accent ml-1">{k.naziv}</span></p>
    ...
    return (
        <div className="space-y-4 animate-in fade-in">
            <div className={`bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border shadow-2xl space-y-4 transition-all ${isEditing ? 'border-amber-500/50' : 'border-theme-border'}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 border-b border-theme-border pb-4">
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">* Naziv Kupca / Firme</label><input value={form.naziv} onChange={e=>handleNazivChange(e.target.value)} disabled={isEditing} className={`w-full p-3 rounded-xl text-xs text-theme-text uppercase font-black outline-none ${isEditing ? 'bg-theme-panel border border-theme-border opacity-50 cursor-not-allowed' : 'bg-theme-panel border border-amber-500/50 focus:border-amber-400'}`} /></div>
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">PDV Broj</label><input value={form.pdv} onChange={e=>setForm({...form, pdv:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-amber-500" /></div>
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Adresa / Lokacija</label><input value={form.adresa} onChange={e=>setForm({...form, adresa:e.target.value})} className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-amber-500" /></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-theme-card p-4 rounded-2xl border border-theme-border space-y-3">
                        ...
                        <div className="flex gap-2 items-end">
                            ...
                            <input type="number" value={tempP_rabat} onChange={e=>setTempP_rabat(e.target.value)} placeholder="%" className="w-16 p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border text-center focus:border-amber-500 outline-none" />
                            <button onClick={addR_Proizvod} className="bg-theme-accent text-theme-text px-3 py-3 rounded-xl font-black text-xs hover:opacity-80">+</button>
                        </div>
                            ...
                            {rabatiProizvodi.map((r, i) => {
                                ...
                                return (
                                    <div key={i} className="text-[10px] text-blue-300 bg-blue-900/20 p-3 rounded-lg flex justify-between items-center border border-blue-500/30">
                                        <div className="flex-1">
                                            <span className="font-black text-theme-text">Šifra: {r.sifra}</span>
                                            {proizvod && (<div className="text-[9px] text-slate-400 mt-1"><span className="text-theme-text font-bold">{proizvod.naziv}</span><br />Dim: {proizvod.visina}x{proizvod.sirina}x{proizvod.duzina} | {proizvod.cijena} KM/{proizvod.default_jedinica}</div>)}
                                        </div>
                    ...
                    <div className="bg-theme-card p-4 rounded-2xl border border-theme-border space-y-3">
                        ...
                        <div className="flex gap-2 items-end">
                            ...
                            <input type="number" value={tempK_rabat} onChange={e=>setTempK_rabat(e.target.value)} placeholder="%" className="w-16 p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border text-center focus:border-amber-500 outline-none" />
                            <button onClick={addR_Kategorija} className="bg-theme-card border-b border-theme-border text-theme-text px-3 py-3 rounded-xl font-black text-xs hover:bg-purple-500">+</button>
                        </div>
                        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                            {rabatiKategorije.map((r, i) => (
                                <div key={i} className="text-[10px] text-purple-300 bg-purple-900/20 p-3 rounded-lg flex justify-between items-center border border-purple-500/30">
                                    <span>Kategorija: <b className="text-theme-text text-xs">{r.kategorija}</b></span>
                ...
                <button onClick={snimiKupca} className={`w-full py-4 text-theme-text font-black rounded-xl uppercase text-xs shadow-lg transition-all ${isEditing ? 'bg-amber-600 hover:bg-amber-500' : 'bg-theme-accent hover:opacity-80'}`}>{isEditing ? '✅ Ažuriraj Kupca' : '➕ Snimi Kupca u Bazu'}</button>
            </div>
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                <h3 className="text-slate-400 font-black uppercase text-[10px] mb-3">Lista Kupaca - Klikni za izmjenu</h3>
                <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                    {kupci.map(k => (
                        <div key={k.id} onClick={() => pokreniIzmjenu(k)} className="flex justify-between items-center p-3 bg-theme-card border border-theme-border rounded-xl cursor-pointer hover:border-amber-500/50 transition-all">
                            <div>
                                <p className="text-theme-text text-xs font-black">{k.naziv}</p>
...
function TabRadnici() {
    ...
    return (
        <div className="space-y-4 animate-in fade-in">
            <div className={`p-6 rounded-box border shadow-2xl space-y-4 transition-all ${editId ? 'bg-blue-950 border-blue-500' : 'bg-theme-card backdrop-blur-[var(--glass-blur)] border-theme-border'}`}>
                <div className="flex justify-between items-center">
                    ...
                    {editId && <button onClick={ponistiEdit} className="text-red-400 hover:text-theme-text text-[10px] font-black uppercase bg-red-900/30 px-3 py-1 rounded-lg">✕ Poništi</button>}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <input placeholder="Ime i Prezime" value={form.ime_prezime} onChange={e=>setForm({...form, ime_prezime:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" />
                    <select value={form.uloga} onChange={e=>setForm({...form, uloga:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500 font-black"><option value="operater">Uloga: Operater (Rad na mašini)</option><option value="admin">Uloga: Administrator (Šef)</option></select>
                    <input placeholder="Username (za login)" value={form.username} onChange={e=>setForm({...form, username:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" />
                    <input placeholder="Password" value={form.password} onChange={e=>setForm({...form, password:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" />
                    <input type="number" placeholder="Bruto satnica (KM)" value={form.bruto_satnica} onChange={e=>setForm({...form, bruto_satnica:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" />
                    <input type="number" placeholder="Dani godišnjeg (početno)" value={form.preostalo_godisnjeg} onChange={e=>setForm({...form, preostalo_godisnjeg:e.target.value})} className="p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" />
                    <div className="col-span-1 md:col-span-2 pt-4 border-t border-theme-border mt-2">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {sviModuli.map(modul => {
                                ...
                                return (<button key={modul} onClick={() => toggleDozvola(modul)} className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${aktivan ? 'bg-emerald-600 border-emerald-400 text-theme-text shadow-[0_0_10px_rgba(16,185,129,0.3)]' : 'bg-theme-panel border-theme-border text-slate-500 hover:border-slate-500'}`}>{aktivan ? '✓ ' : '+ '} {modul}</button>);
                            })}
                        </div>
                    </div>
                </div>
                <div className="flex gap-2 mt-2">
                    <button onClick={snimi} className="flex-1 py-5 bg-theme-accent text-theme-text font-black rounded-xl uppercase text-xs shadow-xl hover:opacity-80 transition-all">{editId ? '✅ Ažuriraj Radnika' : '✅ Snimi Radnika'}</button>
                </div>
            </div>
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                ...
                <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                    {radnici.map(r => (
                        <div key={r.id} className={`flex flex-col md:flex-row justify-between md:items-center p-4 border rounded-2xl gap-3 transition-all ${editId === r.id ? 'bg-blue-900/30 border-blue-500' : 'bg-theme-card border-theme-border hover:border-slate-600'}`}>
                            <div>
                                <p className="text-theme-text text-sm font-black">{r.ime_prezime} <span className="text-slate-500 text-[10px] font-normal">(@{r.username})</span></p>
                                <p className="text-[9px] text-slate-400 uppercase mt-1 mb-2"><span className="bg-blue-900/30 text-theme-accent px-2 py-1 rounded mr-2 border border-blue-500/30">{r.uloga}</span> Satnica: <b className="text-theme-text">{r.bruto_satnica} KM</b> | Godišnji: <b className="text-theme-text">{r.preostalo_godisnjeg} dana</b></p>
                            ...
                            <div className="flex gap-2">
                                <button onClick={() => pokreniEdit(r)} className="text-theme-accent font-black px-4 py-2 hover:bg-theme-accent hover:text-theme-text rounded-xl transition-all border border-blue-500/30 bg-blue-900/20">✎ Uredi</button>
                                <button onClick={async()=>{if(window.confirm(`Da li ste sigurni da zelite obrisati radnika ${r.ime_prezime}?`)){await supabase.from('radnici').delete().eq('id', r.id); load();}}} className="text-red-500 font-black px-4 py-2 hover:bg-red-500 hover:text-theme-text rounded-xl transition-all border border-red-500/30">✕</button>
                            </div>
                        </div>
                    ))}
                </div>
...
function TabKategorijeBlagajne() {
    ...
    return (
        <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-2xl space-y-6 animate-in fade-in">
            ...
            <div className="flex gap-2">
                <select value={form.tip} onChange={e=>setForm({...form, tip:e.target.value})} className="p-4 bg-theme-card rounded-xl text-theme-text font-bold outline-none border border-theme-border"><option value="ULAZ">ULAZ</option><option value="IZLAZ">IZLAZ</option></select>
                <input value={form.naziv} onChange={e=>setForm({...form, naziv:e.target.value})} placeholder="Npr. Kupovina alata..." className="flex-1 p-4 bg-theme-card rounded-xl text-theme-text outline-none border border-theme-border" />
                <button onClick={dodajKategoriju} className="bg-emerald-600 px-6 rounded-xl text-theme-text font-black hover:bg-emerald-500 shadow-lg">Dodaj</button>
            </div>
            <div className="grid grid-cols-2 gap-4 mt-4">
                {['ULAZ', 'IZLAZ'].map(tip => (
                    <div key={tip} className="bg-theme-card p-4 rounded-2xl border border-theme-border">
                        ...
                        {kategorije.filter(k=>k.tip===tip).map(k => (
                            <div key={k.id} className="flex justify-between items-center p-2 border-b border-theme-border">
...
function TabBrending() {
    ...
    return (
        <div className="space-y-6 animate-in fade-in max-w-4xl mx-auto">
            ...
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-blue-500/30 shadow-2xl space-y-4">
                <h3 className="text-blue-500 font-black uppercase text-xs border-b border-theme-border pb-2">🏢 Podaci o firmi (Za PDF Izvještaje)</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Adresa Firme</label><input value={firmaInfo.adresa} onChange={e=>setFirmaInfo({...firmaInfo, adresa:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" placeholder="Rijeka bb, 75328..." /></div>
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Telefon</label><input value={firmaInfo.telefon} onChange={e=>setFirmaInfo({...firmaInfo, telefon:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" placeholder="+387 61..." /></div>
                    <div><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Email</label><input value={firmaInfo.email} onChange={e=>setFirmaInfo({...firmaInfo, email:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-blue-500" placeholder="info@..." /></div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-theme-border">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                        <div className="md:col-span-8">
                            ...
                            <input value={firmaInfo.footer_tekst} onChange={e=>setFirmaInfo({...firmaInfo, footer_tekst:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-amber-500" placeholder="Unesite tekst koji će pisati u dnu svakog dokumenta..." />
                        </div>
                        <div className="md:col-span-2">
                            <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Boja Teksta</label>
                            <div className="flex bg-theme-card rounded-xl border border-theme-border p-1 items-center h-[42px]">
                        ...
                        <div className="md:col-span-2">
                            ...
                            <input type="number" value={firmaInfo.footer_velicina} onChange={e=>setFirmaInfo({...firmaInfo, footer_velicina:e.target.value})} className="w-full p-3 bg-theme-card rounded-xl text-xs text-theme-text outline-none border border-theme-border focus:border-amber-500 text-center" placeholder="12" />
                        </div>
                    </div>
                    {firmaInfo.footer_tekst && (
                        <div className="mt-4 p-4 bg-theme-panel rounded-xl text-center border border-theme-border">
                ...
                <button onClick={spasiFirmuInfo} className="w-full py-3 bg-theme-accent text-theme-text font-black rounded-xl text-xs uppercase shadow-lg hover:opacity-80 transition-all mt-4">💾 Spasi i Sinhronizuj u Bazu</button>
            </div>
            ...
            <div className={`p-6 rounded-box border shadow-2xl space-y-4 transition-all ${isEditing ? 'bg-amber-950/30 border-amber-500' : 'bg-theme-card backdrop-blur-[var(--glass-blur)] border-theme-border'}`}>
                <div className="flex justify-between items-center">
                    ...
                    {isEditing && <button onClick={ponisti} className="text-red-400 hover:text-theme-text text-[10px] font-black uppercase bg-red-900/30 px-3 py-1 rounded-lg transition-all">✕ Poništi</button>}
                </div>
                <div className="grid grid-cols-1 gap-4">
                    <div>
                        <label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">* Naziv Logotipa (npr. Glavni Logo)</label>
                        <input value={form.naziv} onChange={e=>setForm({...form, naziv:e.target.value})} className="w-full p-4 bg-theme-panel rounded-xl text-sm text-theme-text outline-none border border-theme-border focus:border-blue-500 font-bold" placeholder="Unesite naziv..." />
                    </div>
                    ...
                    <div className="bg-theme-card p-4 rounded-2xl border border-theme-border">
                        ...
                        <input type="file" accept="image/png, image/jpeg, image/webp, image/svg+xml, image/x-icon" onChange={e=>setFajlSlike(e.target.files[0])} className="w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:bg-theme-accent file:text-theme-text hover:file:bg-blue-500 cursor-pointer" />
                        ...
                        <div className="mt-4 border-t border-theme-border pt-3">
                            ...
                            <input value={form.url_slike} onChange={e=>setForm({...form, url_slike:e.target.value})} placeholder="https://..." className="w-full p-3 bg-theme-panel rounded-xl text-xs text-slate-400 font-mono outline-none border border-theme-border focus:border-blue-500" disabled={!!fajlSlike} />
                        </div>
                    </div>
                    ...
                    <div className="pt-4 border-t border-theme-border">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {opcijeLokacija.map(lok => {
                                ...
                                return (
                                    <button key={lok} onClick={() => toggleLokacija(lok)} className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase transition-all border ${aktivan ? 'bg-theme-accent border-blue-400 text-theme-text shadow-[0_0_10px_rgba(59,130,246,0.3)] scale-105' : 'bg-theme-panel border-theme-border text-slate-500 hover:border-slate-500'}`}>
                                        {aktivan ? '✓ ' : '+ '} {lok}
                                    </button>
                                );
                            })}
                    ...
                    <div className="bg-theme-card p-4 rounded-xl border border-theme-border flex items-center justify-between mt-2 cursor-pointer hover:bg-theme-panel transition-all" onClick={() => setForm({...form, full_width: !form.full_width})}>
                        <div>
                            <p className="text-theme-text font-black text-xs uppercase">Prikaz preko cijele širine (Memorandum / Baner)</p>
                            <p className="text-[9px] text-slate-400 mt-1">Ako je ovo uključeno, slika ide na sami vrh PDF-a u punoj širini papira.</p>
                        </div>
                        <div className={`w-12 h-6 rounded-box p-1 transition-all ${form.full_width ? 'bg-emerald-500' : 'bg-slate-700'}`}>
                            <div className={`w-4 h-4 bg-white rounded-box transition-all ${form.full_width ? 'translate-x-6' : ''}`}></div>
                        </div>
                    </div>
                </div>
                
                {(form.url_slike || fajlSlike) && (
                    <div className="mt-4 p-6 bg-theme-card rounded-2xl border border-theme-border flex justify-center shadow-inner relative">
                ...
                <button onClick={uploadISnimi} disabled={uploading} className={`w-full py-5 text-theme-text font-black rounded-2xl uppercase text-sm shadow-xl transition-all ${uploading ? 'bg-slate-600 cursor-not-allowed' : (isEditing ? 'bg-amber-600 hover:bg-amber-500' : 'bg-theme-accent hover:opacity-80')}`}>
            ...
            <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-theme-border shadow-xl">
                ...
                <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2">
                    {logotipi.length === 0 && <p className="text-center text-slate-500 text-xs py-6 font-bold border-2 border-dashed border-theme-border rounded-2xl">Još nema dodanih logotipa.</p>}
                    
                    {logotipi.map(l => (
                        <div key={l.id} onClick={() => pokreniIzmjenu(l)} className={`flex flex-col md:flex-row justify-between items-start md:items-center p-4 rounded-2xl cursor-pointer transition-all border ${isEditing && form.id === l.id ? 'bg-amber-900/20 border-amber-500/50' : 'bg-theme-card border-theme-border hover:border-blue-500/50 hover:bg-theme-panel'}`}>
                            <div className="flex items-center gap-4 w-full md:w-auto">
                                <div className="w-20 h-20 bg-theme-panel rounded-2xl flex items-center justify-center p-2 border border-theme-border shrink-0 shadow-inner">
                                    <img src={l.url_slike} className="max-h-full max-w-full object-contain" alt={l.naziv} />
                                </div>
                                <div className="flex-1">
                                    <p className="text-theme-text text-base font-black">{l.naziv}</p>
                                    <div className="flex flex-wrap gap-1 mt-2">
                                        ...
                                        {(l.lokacije_jsonb || []).map(lok => (
                                            <span key={lok} className="text-[8px] bg-blue-900/30 text-theme-accent border border-blue-500/30 px-2 py-1 rounded-md font-bold uppercase">
                                                {lok}
                                            </span>
                                        ))}
                                    </div>
                            ...
                            <div className="mt-4 md:mt-0 flex gap-2 self-end md:self-auto">
                                <button onClick={(e) => obrisi(l.id, l.naziv, e)} className="text-red-500 font-black px-4 py-3 bg-red-900/20 hover:bg-red-500 hover:text-theme-text rounded-xl transition-all border border-red-500/30 text-xs uppercase shadow-md">✕ Obriši</button>
                            </div>
                        </div>
                    ))}
                </div>
...
export default function SettingsModule({ onExit, lockedTab }) {
    ...
    return (
        <div className="p-4 max-w-2xl mx-auto space-y-6">
            <div className="flex justify-between items-center bg-theme-card backdrop-blur-[var(--glass-blur)] p-4 rounded-box border border-theme-border shadow-lg">
                <button onClick={onExit} className={`text-[10px] px-4 py-2 rounded-xl uppercase font-black text-theme-text transition-all ${lockedTab ? 'bg-red-600 hover:bg-red-500' : 'bg-theme-panel hover:bg-slate-700'}`}>
            ...
            {!lockedTab && (
                <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                    {tabs.map(t => (
                        <button key={t} onClick={() => setTab(t)} className={`px-4 py-3 rounded-2xl border-2 whitespace-nowrap transition-all text-[9px] tracking-widest uppercase font-black ${tab === t ? 'bg-theme-accent border-blue-400 text-theme-text shadow-lg' : 'bg-theme-panel border-theme-border text-slate-400 hover:border-slate-500'}`}>{labels[t]}</button>
                    ))}
                </div>
            )}
