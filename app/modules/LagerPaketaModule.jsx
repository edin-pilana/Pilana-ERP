    ...
    return (
        <div className="relative font-black w-full">
            <input
                ...
                className="w-full p-3 bg-[#0f172a] border border-blue-500/50 rounded-xl text-xs text-white outline-none focus:border-blue-400 uppercase font-black tracking-widest shadow-inner"
            />
            {open && search && (
                <div className="absolute top-full left-0 z-[100] w-full mt-1 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto text-left">
                    {filtered.length === 0 && <div className="p-3 text-xs text-slate-500 text-center">Nema rezultata...</div>}
                    {filtered.map(n => (
                        <div key={n.id} onClick={() => handleSelect(n.id)} className="p-3 border-b border-slate-700 hover:bg-blue-600 cursor-pointer transition-colors">
                    ...
                    <div onClick={() => setOpen(false)} className="p-2 text-center text-[9px] text-slate-500 cursor-pointer hover:text-white bg-slate-900 rounded-b-xl uppercase font-bold">Zatvori</div>
                </div>
            )}
        </div>
    );
...
export default function LagerPaketaModule({ onExit, user, header, setHeader }) {
    ...
    return (
        <div className="p-4 max-w-7xl mx-auto space-y-6 font-bold animate-in fade-in">
            ...
            <div className="flex bg-[#1e293b] p-1.5 rounded-3xl border border-slate-700 shadow-xl">
                <button onClick={() => setGlavniTab('paketi')} className={`flex-1 py-4 rounded-2xl text-xs uppercase font-black transition-all flex items-center justify-center gap-2 ${glavniTab === 'paketi' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}>📦 Lager Gotovih Paketa</button>
                <button onClick={() => setGlavniTab('trupci')} className={`flex-1 py-4 rounded-2xl text-xs uppercase font-black transition-all flex items-center justify-center gap-2 ${glavniTab === 'trupci' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}>🪵 Lager Trupaca na Placu</button>
            </div>

            <div className="bg-[#1e293b] p-6 rounded-[2.5rem] border border-slate-700 shadow-2xl space-y-5 relative z-40">
                <div className="flex flex-col md:flex-row gap-4">
                    <div className="flex-1 relative">
                        <input value={pretraga} onChange={e => setPretraga(e.target.value)} placeholder="Brza pretraga po ID-u ili Nazivu..." className="w-full p-4 bg-slate-950 border border-slate-800 rounded-2xl text-white outline-none focus:border-blue-500 font-black uppercase shadow-inner" />
                        <button onClick={() => setIsScanning(true)} className="absolute right-2 top-2 bottom-2 px-4 bg-blue-600 rounded-xl text-white font-bold hover:bg-blue-500 transition-all shadow-lg text-xl">📷</button>
                    </div>
                    {glavniTab === 'paketi' && (
                        <div className="flex bg-slate-900 p-1 rounded-2xl border border-slate-800 shrink-0 overflow-x-auto">
                            {['U PROIZVODNJI', 'NA STANJU', 'OTPREMLJENO'].map(t => (
                                <button key={t} onClick={() => setSubTabPaketi(t)} className={`px-4 py-2 shrink-0 rounded-xl text-[9px] font-black uppercase transition-all ${subTabPaketi === t ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:text-white hover:bg-slate-800'}`}>{t}</button>
                            ))}
                        </div>
                    )}
                </div>
                    ...
                    {glavniTab === 'paketi' ? (
                        ...
                        <>
                            <div className="col-span-2 md:col-span-1">
                                ...
                                <select value={fTrupac.sumarija} onChange={e=>setFTrupac({...fTrupac, sumarija: e.target.value})} className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white outline-none focus:border-amber-500 uppercase font-black cursor-pointer">
                            ...
                            <div className="col-span-2 md:col-span-1">
                                ...
                                <select value={fTrupac.vrsta} onChange={e=>setFTrupac({...fTrupac, vrsta: e.target.value})} className="w-full p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white outline-none focus:border-amber-500 uppercase font-black cursor-pointer">
                                    <option value="">SVE VRSTE...</option>
                                    {vrsteDrvetaList.map(v => <option key={v} value={v}>{v}</option>)}
                                </select>
                            </div>
                            <div><label className="text-[8px] text-slate-500 uppercase ml-2 mb-1 block">Prečnik (od-do)</label><div className="flex gap-1"><input type="number" value={fTrupac.precnikOd} onChange={e=>setFTrupac({...fTrupac, precnikOd: e.target.value})} placeholder="Min" className="w-1/2 p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white text-center outline-none focus:border-amber-500" /><input type="number" value={fTrupac.precnikDo} onChange={e=>setFTrupac({...fTrupac, precnikDo: e.target.value})} placeholder="Max" className="w-1/2 p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white text-center outline-none focus:border-amber-500" /></div></div>
                            <div><label className="text-[8px] text-slate-500 uppercase ml-2 mb-1 block">Dužina (od-do)</label><div className="flex gap-1"><input type="number" value={fTrupac.duzinaOd} onChange={e=>setFTrupac({...fTrupac, duzinaOd: e.target.value})} placeholder="Min" className="w-1/2 p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white text-center outline-none focus:border-amber-500" /><input type="number" value={fTrupac.duzinaDo} onChange={e=>setFTrupac({...fTrupac, duzinaDo: e.target.value})} placeholder="Max" className="w-1/2 p-3 bg-slate-900 border border-slate-700 rounded-xl text-xs text-white text-center outline-none focus:border-amber-500" /></div></div>
                        </>
                    )}
                    <div className="flex flex-col justify-end">
                        <button onClick={() => { setPretraga(''); setFPaket({ rn: '', debljinaOd: '', debljinaDo: '', sirinaOd: '', sirinaDo: '', duzinaOd: '', duzinaDo: '' }); setFTrupac({ sumarija: '', klasa: '', precnikOd: '', precnikDo: '', duzinaOd: '', duzinaDo: '', vrsta: '' }); }} className="w-full p-3 bg-slate-800 text-slate-400 rounded-xl text-[9px] uppercase font-black hover:bg-red-900/30 hover:text-red-400 transition-all border border-slate-700">Poništi filtere ✕</button>
                    </div>
                </div>
            </div>

            {glavniTab === 'paketi' && fPaket.rn && rnDetalji && (
                <div className="bg-purple-900/20 border-2 border-purple-500/50 p-6 rounded-[2.5rem] shadow-2xl animate-in zoom-in-95 relative z-30">
                    <div className="flex justify-between items-start mb-4 border-b border-purple-500/30 pb-3">
                        <div>
                            <h3 className="text-purple-400 font-black uppercase text-sm tracking-widest">Pregled realizacije: {fPaket.rn}</h3>
                            <p className="text-white text-xs mt-1">Kupac: <span className="font-bold">{rnDetalji.kupac}</span></p>
                        </div>
                        <span className={`text-[10px] px-3 py-1 rounded font-black uppercase ${rnDetalji.status === 'ZAVRŠENO' ? 'bg-emerald-900/40 text-emerald-400' : 'bg-purple-900/40 text-purple-400'}`}>{rnDetalji.status}</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {rnDetalji.stavke.map((st, i) => (
                            <div key={i} className="bg-slate-900 p-4 rounded-2xl border border-slate-700 shadow-md">
                                <p className="text-white text-xs font-black mb-2">{st.sifra} - {st.naziv}</p>
                                <div className="space-y-1">
                                    <div className="flex justify-between text-[10px]"><span className="text-slate-400">Naručeno:</span><span className="text-white font-bold">{st.kolicina_obracun} m³ <span className="text-slate-500">({st.kolicina_unos} {st.jm_unos})</span></span></div>
                                    ...
                                    <div className="flex justify-between text-[10px] pt-1 border-t border-slate-800 mt-1"><span className="text-slate-400 font-bold">Preostalo za izradu:</span><span className={st.faliM3 > 0 ? "text-amber-400 font-black" : "text-emerald-500 font-black"}>{st.faliM3 > 0 ? `${st.faliM3.toFixed(3)} m³ (${st.faliKom} ${st.jm_unos})` : 'ZAVRŠENO ✅'}</span></div>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="flex justify-between items-center bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-inner">
                <p className="text-slate-500 text-xs uppercase">Prikazano rezultata: <span className="text-white font-black ml-1">{stats.count}</span></p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {loading ? (
                    ...
                    <div className="col-span-full text-center py-20 text-slate-600 font-bold border-2 border-dashed border-slate-800 rounded-[2.5rem]">Nema rezultata za odabrane filtere.</div>
                ) : (
                    (glavniTab === 'paketi' ? filtriraniPaketi : filtriraniTrupci).map(item => (
                        <div key={item.id} onClick={() => { if(glavniTab==='paketi') ucitajHistoriju(item); }} className={`p-5 rounded-[2rem] border transition-all shadow-xl group relative overflow-hidden ${glavniTab === 'paketi' ? 'bg-[#111827] border-slate-800 hover:border-blue-500 hover:-translate-y-1 cursor-pointer' : 'bg-[#1e1e1e] border-slate-800'}`}>
                            {glavniTab === 'paketi' ? (
                                <>
                                    ...
                                    <div className="flex justify-between items-start mb-3">
                                        <div className="flex-1 pr-2">
                                            <p className="text-blue-400 font-black text-lg">{item.paket_id}</p>
                                            ...
                                            <div className="flex flex-wrap gap-1 mt-2">
                                                ...
                                                {item.oznake && item.oznake.length > 0 && item.oznake.map((oznaka, idx) => (
                                                    <span key={idx} className="text-[8px] bg-slate-800 text-slate-300 px-2 py-0.5 rounded uppercase font-bold border border-slate-700">{oznaka}</span>
                                                ))}
                                            </div>
                                        </div>
                                        <div className="text-right flex flex-col items-end shrink-0">
                                            ...
                                            <div className="bg-slate-800 border border-slate-700 px-3 py-1.5 rounded-xl text-right min-w-[80px]">
                                                <p className="text-white font-black text-sm leading-none">{item.kolicina_ulaz || 0} <span className="text-[9px] text-slate-400 uppercase">{item.jm || 'kom'}</span></p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="flex justify-between items-center mt-4 pt-3 border-t border-slate-800/50">
                                        ...
                                        <span className="text-[9px] text-blue-400 opacity-0 group-hover:opacity-100 transition-all uppercase font-black bg-blue-900/30 px-2 py-1 rounded">Vidi Detalje 🔍</span>
                                    </div>
                                </>
                            ) : (
                                <>
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            ...
                                            <p className="text-white text-xs font-bold mt-1 uppercase">{item.vrsta_drveta || 'Miješano'}</p>
                                    ...
                                    <div className="mt-4 pt-3 border-t border-slate-800/50 flex justify-between items-center">
                                        ...
                                        <span className={`text-[8px] px-2 py-1 rounded font-black uppercase ${item.klasa === 'I' ? 'bg-emerald-900/30 text-emerald-400' : 'bg-slate-800 text-slate-400'}`}>Klasa {item.klasa}</span>
                                    </div>
                                </>
                            )}
                        </div>
            ...
            {selektovaniPaket && (
                <div className="fixed inset-0 z-[150] bg-black/90 flex items-center justify-center p-4 backdrop-blur-md animate-in fade-in">
                    <div className="bg-[#1e293b] border-2 border-blue-500 p-8 rounded-[3rem] shadow-2xl max-w-2xl w-full relative max-h-[90vh] overflow-y-auto">
                        ...
                        <div className="flex gap-4 mt-4 mb-8 bg-slate-900 p-4 rounded-2xl border border-slate-700">
                            <div className="flex-1">
                                <p className="text-[10px] text-slate-500 uppercase font-bold">Količina (Unos)</p>
                                <p className="text-white font-black text-xl">{selektovaniPaket.kolicina_ulaz || 0} <span className="text-xs text-slate-400">{selektovaniPaket.jm || 'kom'}</span></p>
                            </div>
                        ...
                        {selektovaniPaket.ai_sirovina_ids && selektovaniPaket.ai_sirovina_ids.length > 0 && (
                            <div className="mb-6 bg-blue-900/10 border border-blue-500/30 p-4 rounded-2xl">
                                <h4 className="text-blue-400 font-black uppercase text-[10px] mb-3 flex items-center gap-2">⬅️ Sljedivost unazad (Izvor)</h4>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {(Array.isArray(selektovaniPaket.ai_sirovina_ids) ? selektovaniPaket.ai_sirovina_ids : [selektovaniPaket.ai_sirovina_ids]).map(srcId => (
                                        <button key={srcId} onClick={() => otvoriPaketDirektno(srcId)} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg hover:bg-blue-500 transition-all cursor-pointer hover:scale-105 border border-blue-400">
                                            {srcId}
                                        </button>
                                    ))}
                                </div>
                        ...
                        {izvedeniPaketi.length > 0 && (
                            <div className="mb-6 bg-purple-900/10 border border-purple-500/30 p-4 rounded-2xl">
                                <h4 className="text-purple-400 font-black uppercase text-[10px] mb-3 flex items-center gap-2">➡️ Sljedivost unaprijed (Proizvedeno)</h4>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {izvedeniPaketi.map(izv => (
                                        <button key={izv.paket_id} onClick={() => otvoriPaketDirektno(izv.paket_id)} className="bg-purple-600 text-white px-4 py-2 rounded-xl text-[10px] font-black shadow-lg hover:bg-purple-500 transition-all cursor-pointer hover:scale-105 flex flex-col items-start border border-purple-400">
                        ...
                        <div className="space-y-4 mb-8">
                            <h4 className="text-slate-500 font-black uppercase text-[10px] border-b border-slate-700 pb-2">Vremenska Sljedivost (Historija izmjena):</h4>
                                ...
                                <div className="space-y-4">
                                    {historija.map((h, i) => (
                                        <div key={i} className="flex gap-4 items-start relative before:absolute before:left-[7px] before:top-5 before:bottom-[-20px] before:w-[2px] before:bg-slate-800 last:before:hidden">
                                            <div className="w-4 h-4 rounded-full bg-slate-700 border-4 border-[#1e293b] z-10 shrink-0 mt-1"></div>
                                            <div>
                        ...
                        <button 
                            ...
                            disabled={isPrinting}
                            className={`w-full py-5 text-white font-black rounded-2xl uppercase shadow-[0_0_15px_rgba(37,99,235,0.4)] transition-all flex items-center justify-center gap-2 ${isPrinting ? 'bg-slate-600' : 'bg-blue-600 hover:bg-blue-500'}`}
                        >
