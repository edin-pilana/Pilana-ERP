...
<div className="max-w-[1600px] mx-auto flex flex-col lg:flex-row justify-between items-center p-4 rounded-2xl shadow-xl mb-6 gap-4 border print:hidden" style={{ backgroundColor: saas.ui.boja_kartice, borderColor: '#1e293b' }}>
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6 w-full lg:w-auto overflow-x-auto">
                    <div className="flex items-center gap-4">
                        <button onClick={onExit} className="bg-slate-800 text-slate-400 hover:bg-red-500 hover:text-white px-4 h-10 rounded-xl text-[10px] font-black uppercase transition-all shadow-md shrink-0">← Meni</button>
                    </div>
                    ...
                    <nav className="flex gap-2 bg-black/20 p-1.5 rounded-xl border border-white/5 overflow-x-auto whitespace-nowrap shadow-inner">
                        {['prorez', 'dorada', 'ukupno', 'finansije', 'ai kontrolor'].map(t => (
                            <button key={t} onClick={() => setActiveTab(t)} className={`px-4 py-2 rounded-lg text-[11px] font-bold uppercase transition-all ${activeTab === t ? 'text-white shadow-lg' : 'text-slate-500 hover:text-white hover:bg-white/5'}`} style={{ backgroundColor: activeTab === t ? saas.ui.boja_akcenta : 'transparent' }}>
                                {t}
                            </button>
                        ))}
                    </nav>
                ...
                <div className="flex flex-col md:flex-row items-center gap-4 w-full lg:w-auto justify-end">
                    ...
                    {(activeTab === 'prorez' || activeTab === 'dorada') && (
                       <select value={masinaFilter} onChange={e => setMasinaFilter(e.target.value)} className="bg-black/20 text-slate-300 p-2 h-10 rounded-xl text-xs font-bold border border-white/5 outline-none focus:border-emerald-500 shadow-inner uppercase tracking-widest [&>option]:bg-slate-900 [&>option]:text-white">
                    ...
                    <Flex className="bg-black/20 p-1.5 rounded-xl border border-white/5 gap-1 shadow-inner w-full md:w-auto">
                        <button onClick={() => { setTipDatuma('dan'); setIsPeriodic(false); setDatumDo(datumOd); }} className={`px-3 py-1.5 rounded-lg text-[10px] font-bold uppercase transition-all ${!isPeriodic?'bg-slate-700 text-white':'text-slate-500 hover:text-white'}`}>Dan</button>
                    ...
                    <Flex className="bg-black/20 p-1.5 rounded-xl border border-white/5 gap-2 shadow-inner w-full md:w-auto flex-wrap">
                        {!isPeriodic ? (
                            <>
                                ...
                                <div className="flex items-center justify-center bg-slate-900 px-3 py-1.5 rounded border border-slate-700 h-8">
                            ...
                            <>
                                <input type="date" value={datumOd} onChange={e=>setDatumOd(e.target.value)} className="bg-slate-900 border border-slate-700 text-slate-300 font-bold text-xs p-1.5 rounded h-8 outline-none [&::-webkit-calendar-picker-indicator]:invert" />
                                <span className="text-slate-600 font-black">-</span>
                                <input type="date" value={datumDo} onChange={e=>setDatumDo(e.target.value)} className="bg-slate-900 border border-slate-700 text-slate-300 font-bold text-xs p-1.5 rounded h-8 outline-none [&::-webkit-calendar-picker-indicator]:invert" />
                                <div className="flex gap-1 ml-2 border-l border-slate-800 pl-2">
                                    <button onClick={()=>setBrziDatum('7d')} className="px-2 py-1.5 bg-slate-800 hover:bg-blue-600 hover:text-white rounded text-[9px] font-black uppercase text-slate-300 transition-colors">7 Dana</button>
                                    <button onClick={()=>setBrziDatum('mjesec')} className="px-2 py-1.5 bg-slate-800 hover:bg-blue-600 hover:text-white rounded text-[9px] font-black uppercase text-slate-300 transition-colors">Ovaj Mj</button>
                                </div>
                            </>
                        )}
                    </Flex>
                    ...
                    {user?.uloga === 'superadmin' && (
                        <button onClick={saas.pokreniEdit} className="flex items-center gap-2 px-3 py-2 bg-amber-500/10 text-amber-500 border border-amber-500/30 rounded-xl text-[9px] font-black uppercase hover:bg-amber-500 hover:text-white transition-all">
                            <Settings size={14} /> UI
                        </button>
                    )}
                </div>
            ...
            {saas.isEditMode && (
                <div className="max-w-[1600px] mx-auto mb-6 bg-amber-900/20 p-6 rounded-3xl flex flex-wrap gap-4 items-center justify-between border-2 border-amber-500/50 shadow-2xl">
                    <div>
                    ...
                    <div className="flex gap-2">
                        <button onClick={saas.odustani} className="px-6 py-3 bg-red-900/40 text-red-400 border border-red-500/50 rounded-xl text-[9px] font-black uppercase hover:bg-red-500 hover:text-white transition-all shadow-md">✕ Odustani</button>
                        <button onClick={saas.spasiDizajn} className="px-6 py-3 bg-emerald-600 text-white rounded-xl text-[9px] font-black uppercase shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:bg-emerald-500 transition-all">💾 Spasi Dizajn</button>
                    </div>
                </div>
            )}
