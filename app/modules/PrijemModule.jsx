...
export default function PrijemModule({ user, header, setHeader, onExit }) {
    ...
    const [form, setForm] = useState({ 
        broj_plocice: '', redni_broj: '', vrsta: '', klasa: '', 
    ...
    useEffect(() => {
        ...
        supabase.from('vrste_drveta').select('naziv').then(({data}) => {
            if(data && data.length > 0) { setVrsteList(data.map(d=>d.naziv)); setForm(f => ({...f, vrsta: data[0].naziv})); }
        });
        supabase.from('klase_trupaca').select('naziv').then(({data}) => {
            if(data && data.length > 0) { setKlaseList(data.map(d=>d.naziv)); setForm(f => ({...f, klasa: data[0].naziv})); }
        });
    ...
    const snimiTrupac = async () => {
        ...
        // Cijena prevoza sada ide iz podružnice!
        ...
        setScan(''); 
        setForm({ broj_plocice: '', redni_broj: '', vrsta: vrsteList[0] || '', klasa: klaseList[0] || '', duzina: '', promjer: '', isKontrola: false, kontrolna_duzina: '', kontrolni_promjer: '' }); 
    ...
    const renderPolje = (polje, formData, setFormData) => {
        ...
        if (actualId === 'vrsta') return (
            <select value={val} onChange={e => setVal(e.target.value)} className="w-full h-full min-h-[45px] px-3 py-0 bg-theme-panel border border-theme-border rounded-xl text-theme-text outline-none uppercase text-center text-sm font-black shadow-inner cursor-pointer">
                {vrsteList.map(v => <option key={v} value={v} className="bg-slate-800 text-white">{v}</option>)}
            </select>
        );
        if (actualId === 'klasa') return (
            <select value={val} onChange={e => setVal(e.target.value)} className="w-full h-full min-h-[45px] px-3 py-0 bg-theme-panel border border-theme-border rounded-xl text-theme-text outline-none uppercase text-center text-sm font-black shadow-inner cursor-pointer">
                {klaseList.map(k => <option key={k} value={k} className="bg-slate-800 text-white">{k}</option>)}
            </select>
        );
    ...
    return (
        <div className="p-4 max-w-7xl mx-auto space-y-6 font-sans animate-in fade-in pb-24 overflow-visible">
            <MasterHeader header={header} setHeader={setHeader} onExit={onExit} user={user} hideMasina={true} modulIme="prijem" saas={saas} />

            <div className="text-center mb-6">
                <h2 className="text-theme-accent font-black tracking-widest uppercase text-xl md:text-2xl drop-shadow-md flex items-center justify-center gap-3">🌳 PRIJEM TRUPACA (Lager)</h2>
            </div>
                    ...
                    <div>
                        ...
                        <div className="text-xs text-theme-muted mt-2 flex flex-wrap gap-4 uppercase font-black tracking-widest">
                            ...
                            {parseFloat(totalRazlikaM3) !== 0 && (
                                <p>Stvarno: <span className="text-theme-text text-sm ml-1">{totalStvarno} m³</span> | 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 overflow-visible">
                <div className="lg:col-span-5 space-y-6 overflow-visible">
                    <div className="p-6 rounded-[var(--radius-box)] shadow-2xl transition-all border border-theme-border bg-theme-card backdrop-blur-[var(--glass-blur)] relative overflow-visible">
                        <div className="text-center font-black relative mb-6 border-b border-theme-border pb-4">
                            <span className="text-xs text-theme-accent uppercase tracking-widest flex items-center justify-center gap-2"><span className="w-2 h-2 rounded-full bg-theme-accent"></span> {saas.ui.naslov_zaglavlja}</span>
                        </div>
                ...
                {/* OVDJE JE UKLONJEN OVERFLOW-HIDDEN ZBOG MENIJA */}
                <div className="lg:col-span-7 space-y-6 overflow-visible">
                    <div className={`p-8 rounded-[var(--radius-box)] shadow-[0_0_40px_rgba(0,0,0,0.3)] border bg-theme-card backdrop-blur-[var(--glass-blur)] relative overflow-visible group border-theme-accent/40`}>
                        ...
                        <label className="text-xs uppercase text-theme-accent block mb-4 font-black tracking-widest text-center">{saas.ui.naslov_skenera || "SKENIRAJ QR KOD TRUPCA"}</label>
                        ...
                        <div className="flex items-center justify-between bg-black/20 p-5 rounded-2xl border border-amber-500/30 mb-4">
                            <div><span className="text-xs text-amber-500 uppercase font-black tracking-widest">Kontrola Dimenzija</span><p className="text-[10px] text-slate-400 mt-1">Omogući unos kontrolnih mjera (Kalo)</p></div>
                        ...
                        {form.isKontrola && (
                            <div className="grid grid-cols-2 gap-4 mb-6 p-6 bg-amber-900/10 border border-amber-500/30 rounded-2xl animate-in zoom-in-95">
                                <div><label className="text-[9px] text-amber-500 uppercase block mb-2 text-center font-black tracking-widest">Stvarna Dužina (M)</label><input type="number" value={form.kontrolna_duzina} onChange={e => setForm({...form, kontrolna_duzina: e.target.value})} className="w-full p-4 bg-theme-panel border border-amber-500/50 rounded-xl text-amber-400 outline-none focus:border-amber-400 text-center text-lg font-black shadow-inner" placeholder="0" /></div>
                                <div><label className="text-[9px] text-amber-500 uppercase block mb-2 text-center font-black tracking-widest">Stvarni Prečnik (CM)</label><input type="number" value={form.kontrolni_promjer} onChange={e => setForm({...form, kontrolni_promjer: e.target.value})} className="w-full p-4 bg-theme-panel border border-amber-500/50 rounded-xl text-amber-400 outline-none focus:border-amber-400 text-center text-lg font-black shadow-inner" placeholder="0" /></div>
                                <div className="col-span-2 text-center mt-2 flex justify-between items-center bg-black/30 p-4 rounded-xl border border-amber-500/20 shadow-inner"><span className="text-[10px] text-theme-muted uppercase font-black tracking-widest">Stvarna Kubikaža: </span><span className="text-amber-500 font-black text-2xl">{calculatedKontrolnaZapremina} m³</span></div>
                            </div>
                        )}
                                ...
                                <div key={t.id} className={`flex flex-col p-5 bg-theme-panel border rounded-2xl shadow-md transition-all hover:scale-[1.01] ${isKontrolisan ? 'border-amber-500/40' : 'border-theme-border hover:border-theme-accent/50'}`}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            ...
                                            <div className="text-[10px] text-theme-muted uppercase mt-2 font-bold tracking-widest">{t.vrsta} | Klasa {t.klasa} | Papir: L:{t.duzina}m Ø:{t.promjer}cm</div>
                                            {parseFloat(t.nabavna_vrijednost || 0) > 0 && <div className="text-[10px] text-emerald-500 uppercase mt-1 font-black">Zaduženje: {parseFloat(t.nabavna_vrijednost).toLocaleString('bs-BA')} KM</div>}
                                        </div>
                                    ...
                                    {isKontrolisan && (
                                        <div className="mt-4 pt-3 border-t border-amber-500/20 flex justify-between items-center bg-amber-900/10 p-3 rounded-xl shadow-inner">
                                            <div className="text-[10px] text-amber-500 uppercase font-black tracking-widest">Stvarno: L:{t.kontrolna_duzina}m Ø:{t.kontrolni_promjer}cm = {t.kontrolna_zapremina} m³</div>
                                            <div className={`text-sm font-black bg-theme-card px-3 py-1 rounded-lg shadow-sm ${razlika > 0 ? 'text-emerald-400' : (razlika < 0 ? 'text-red-500' : 'text-slate-400')}`}>{razlika > 0 ? '+' : ''}{razlika} m³</div>
                                        </div>
                                    )}
                                </div>
                            );
