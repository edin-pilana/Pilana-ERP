...
function RN_SearchablePonuda({ ponude, value, onChange, onScanClick }) {
    ...
    return (
        <div className="relative font-black w-full flex bg-theme-panel border-2 border-blue-500 rounded-xl overflow-visible focus-within:border-blue-400 transition-all shadow-inner">
            ...
            <button 
                onClick={onScanClick} 
                className="px-6 bg-theme-accent text-theme-text text-xl hover:opacity-80 transition-all border-l border-blue-500/50 relative z-10"
            >
                📷
            </button>
                ...
                <div className="absolute top-full left-0 z-[100] w-full mt-2 bg-theme-panel border border-theme-border rounded-xl shadow-2xl max-h-60 overflow-y-auto text-left">
                    ...
                    {filtered.map(p => (
                        <div 
                            ...
                            className="p-3 border-b border-theme-border hover:bg-theme-accent cursor-pointer transition-colors"
                        >
                    ...
                    <div 
                        ...
                        className="p-2 text-center text-[9px] text-slate-500 cursor-pointer hover:text-theme-text bg-theme-card rounded-b-xl uppercase font-bold tracking-widest mt-1"
                    >
                        Zatvori
                    </div>
                </div>
    ...
    return (
        <div className="relative font-black w-full">
            <input 
                ...
                className="w-full p-4 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-purple-500 shadow-inner" 
            />
            {open && filtered.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-theme-panel border border-theme-border rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                    {filtered.map(k => {
                        ...
                        return (
                            <div 
                                ...
                                className="p-3 border-b border-theme-border hover:bg-theme-card border-b border-theme-border cursor-pointer transition-all"
                            >
                    ...
                    <div 
                        onClick={() => setOpen(false)} 
                        className="p-2 text-center text-[8px] text-slate-500 cursor-pointer hover:text-theme-text bg-theme-card sticky bottom-0"
                    >
                        Zatvori
                    </div>
                </div>
...
export default function RadniNaloziModule({ user, header, setHeader, onExit }) {
    ...
    // NOVO: Dodan 'tip_naloga' u osnovnu formu
    const [form, setForm] = useState({ 
        ...
        tip_naloga: 'OBIČNI', // Po defaultu obična proizvodnja
        modifikovan: false 
    });
    ...
    const snimiNalog = async () => {
        ...
        const payload = {
            ...
            stavke_jsonb: stavke, tehnologija_jsonb: form.tip_naloga === 'FAZNI' ? tehnologija : {}, status: form.status, 
            modifikovan: form.modifikovan, tip_naloga: form.tip_naloga, // NOVO
        ...
        if (window.confirm("✅ Nalog snimljen! Da li želite PDF?")) {
            // Kreira direktni pdf ako nije fazni nalog
            form.tip_naloga === 'FAZNI' ? setTab('lista') : kreirajPDF(); 
        }
    ...
    const resetFormu = () => {
        setForm({ id: generisiID(), broj_ponude: '', kupac_naziv: '', datum: new Date().toISOString().split('T')[0], rok_isporuke: '', napomena: '', status: 'U PROIZVODNJI', tip_naloga: 'OBIČNI', modifikovan: false });
    ...
    const pokreniIzmjenuNaloga = (n) => {
        setForm({ 
            ...
            tip_naloga: n.tip_naloga || 'OBIČNI',
    ...
    const renderPoljeHeader = (polje) => {
        if (polje.id === 'ponuda') return <input value={form.broj_ponude} onChange={e=>setForm({...form, broj_ponude: e.target.value})} className="w-full h-full min-h-[45px] p-4 bg-theme-card rounded-xl text-theme-text outline-none border border-theme-border uppercase focus:border-purple-500" placeholder="Npr. PON-2026..." />;
        ...
        if (polje.id === 'datum') return <input type="date" value={form.datum} onChange={e=>setForm({...form, datum:e.target.value})} className="w-full h-full min-h-[45px] p-4 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border font-black outline-none" />;
        if (polje.id === 'rok') return <input type="date" value={form.rok_isporuke} onChange={e=>setForm({...form, rok_isporuke: e.target.value})} className="w-full h-full min-h-[45px] p-4 bg-theme-panel rounded-xl text-theme-text border border-theme-border outline-none" />;
    ...
    <button 
        ...
        className={`flex-1 py-3 rounded-xl text-[10px] uppercase font-black transition-all ${tab === 'novi' ? 'bg-theme-accent text-white shadow-lg' : 'text-theme-muted hover:bg-theme-card hover:text-theme-text'}`}
    >
        ➕ Glavni Nalog
    </button>
    <button 
        onClick={otvoriRazradu} 
        className={`flex-1 py-3 rounded-xl text-[10px] uppercase font-black transition-all ${tab === 'razrada' ? 'bg-theme-accent text-white shadow-lg' : 'text-theme-muted hover:bg-theme-card hover:text-theme-text'}`}
    >
        🧪 Razrada Tehnologije
    </button>
    <button 
        onClick={() => setTab('lista')} 
        className={`flex-1 py-3 rounded-xl text-[10px] uppercase font-black transition-all ${tab === 'lista' ? 'bg-theme-accent text-white shadow-lg' : 'text-theme-muted hover:bg-theme-card hover:text-theme-text'}`}
    >
        📋 Arhiva
    </button>
</div>
                    ...
                    <div className={`p-6 rounded-box border-2 shadow-2xl space-y-4 transition-all relative z-[40] ${saas.isEditMode ? 'border-dashed border-amber-500 bg-black/20' : (form.modifikovan ? 'border-amber-500 bg-theme-card backdrop-blur-[var(--glass-blur)]' : 'border-purple-500/30 bg-theme-card backdrop-blur-[var(--glass-blur)]')}`} >
                        <div className="flex justify-between items-center mb-4 pb-4 border-b border-theme-border/50">
                            ...
                            <div className="flex gap-2 self-start">
                                {isEditingNalog && (
                                    <button onClick={kreirajPDF} className="text-[10px] bg-theme-panel text-theme-text border border-slate-600 px-4 py-2 rounded-xl uppercase hover:bg-white hover:text-black transition-all shadow-md font-black">
                                        🖨️ Isprintaj PDF
                                    </button>
                                )}
                        ...
                        {form.modifikovan && (
                            <div className="bg-amber-900/40 border border-amber-500 p-4 rounded-2xl flex items-center gap-3 animate-pulse mb-4">
                                ...
                                <div className="text-[10px] text-amber-400 uppercase font-black leading-tight">
                                    ...
                                    <span className="text-theme-text font-normal mt-1 block">Ova informacija će biti poslana u računovodstvo za otpremnicu i račun!</span>
                                </div>
                            </div>
                        )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-4 gap-3 items-start">
                            {aktivnaPolja.map((polje, index) => (
                                ...
                                >
                                    ...
                                    {saas.isEditMode ? (
                                        <input value={polje.label} onChange={(e) => updatePolje(index, 'label', e.target.value)} className="w-full bg-theme-card text-amber-400 p-1 mb-1 rounded border border-amber-500/50 text-[8px] uppercase font-black text-center shrink-0" placeholder="Naslov polja" />
                                    ) : (
                    ...
                    <div className="bg-theme-card p-6 rounded-box border border-theme-border space-y-4 shadow-xl relative z-[30]">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                            ...
                            <div className="md:col-span-4 flex gap-2">
                                <input 
                                    ...
                                    placeholder="Kol." 
                                    className="w-1/2 min-w-0 p-4 bg-black rounded-xl text-lg text-theme-text font-black text-center border border-theme-border outline-none focus:border-purple-500" 
                                />
                                <select 
                                    value={stavkaForm.jm_unos} 
                                    onChange={e=>setStavkaForm({...stavkaForm, jm_unos:e.target.value})} 
                                    className="w-1/2 min-w-0 p-4 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none cursor-pointer"
                                >
                            ...
                            <div className="md:col-span-2">
                                <button 
                                    onClick={dodajStavku} 
                                    className="w-full h-full min-h-[56px] bg-theme-accent text-theme-text p-4 rounded-xl font-black hover:opacity-80 shadow-lg transition-all flex items-center justify-center"
                                >
                                    DODAJ +
                                </button>
                            </div>
                        ...
                        {trenutniProizvod && stavkaForm.kolicina_unos && (
                            <div className="text-[10px] text-emerald-400 bg-emerald-900/20 p-2 rounded-lg border border-emerald-500/20 text-center uppercase animate-in zoom-in-95">
                                Automatski preračunato: <span className="text-theme-text text-sm ml-1 font-black">{stavkaForm.kolicina_obracun} m³</span>
                            </div>
                        )}
                    </div>
                    ...
                    {stavke.length > 0 && (
                        <div className="bg-theme-card backdrop-blur-[var(--glass-blur)] p-6 rounded-box border border-purple-500/30 shadow-2xl animate-in slide-in-from-bottom">
                            <div className="flex justify-between items-center mb-4">
                                <h3 className="text-theme-accent font-black uppercase text-xs">3. Pregled stavki naloga</h3>
                                ...
                                {stavke.map((s, i) => (
                                    <div key={s.id} onClick={() => urediStavku(s)} className="flex justify-between items-center p-4 bg-theme-card border border-theme-border rounded-xl cursor-pointer hover:border-purple-500 transition-all group shadow-md">
                                        ...
                                        <div className="text-right flex items-center gap-4">
                                            <div>
                                                <p className="text-theme-accent text-lg font-black tracking-widest">{s.kolicina_obracun} m³</p>
                            ...
                            <textarea 
                                ...
                                className="w-full mt-4 p-4 bg-theme-card border border-theme-border rounded-xl text-xs text-theme-text outline-none focus:border-purple-500 shadow-inner" 
                                rows="3"
                            ></textarea>
                            <button 
                                onClick={snimiNalog} 
                                className={`w-full mt-4 py-6 text-theme-text font-black rounded-box uppercase shadow-[0_0_20px_rgba(168,85,247,0.4)] transition-all text-sm tracking-widest ${isEditingNalog ? 'bg-amber-600 hover:bg-amber-500 shadow-[0_0_20px_rgba(245,158,11,0.4)]' : 'bg-theme-card border-b border-theme-border hover:bg-purple-500'}`}
                            >
            ...
            {tab === 'razrada' && (
                <div className="space-y-6 animate-in slide-in-from-right max-w-4xl mx-auto">
                    {form.tip_naloga === 'OBIČNI' ? (
                        <div className="bg-theme-card border border-amber-500/50 p-10 rounded-box text-center shadow-2xl animate-in fade-in">
                            <span className="text-5xl block mb-4">ℹ️</span>
                            <h2 className="text-xl text-amber-400 font-black uppercase">Obična Proizvodnja</h2>
                            <p className="text-slate-400 mt-3 text-sm">Ovaj radni nalog je označen kao "Obična (Direktna)" proizvodnja.<br/>Da biste definisali faze obrade, vratite se na Glavni Nalog i promijenite tip u <b className="text-amber-500">"Fazna (BOM)"</b>.</p>
                            <button onClick={() => setTab('novi')} className="mt-6 bg-theme-panel hover:bg-white hover:text-black text-slate-300 px-6 py-3 rounded-xl font-black uppercase transition-all shadow-lg border border-slate-600">← Vrati se na nalog</button>
                        </div>
                    ) : (
                                    ...
                                    <div key={s.id} className="bg-theme-card p-6 rounded-box border border-theme-border flex flex-col gap-4 shadow-lg">
                                        <div className="flex justify-between items-center border-b border-theme-border pb-3">
                                            <div>
                                                ...
                                                <span className="text-theme-accent text-base font-black">{s.sifra} - {s.naziv}</span>
                                            </div>
                                            <div className="text-right">
                                                <span className="text-slate-500 text-[10px] block uppercase">Ciljna količina:</span>
                                                <span className="text-theme-text font-black text-xl">{s.kolicina_unos} <span className="text-sm text-slate-400">{s.jm_unos}</span></span>
                                            </div>
                                        </div>
                                                ...
                                                <div key={faza.id} className="flex flex-col lg:flex-row gap-3 items-end bg-theme-panel p-5 rounded-2xl border border-theme-border relative group flex-wrap">
                                                    ...
                                                    <div className="flex-1 w-full min-w-[140px]">
                                                        ...
                                                        <select 
                                                            ...
                                                            className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none uppercase font-bold focus:border-amber-500 cursor-pointer"
                                                        >
                                                    ...
                                                    <div className="flex-1 w-full min-w-[140px]">
                                                        ...
                                                        <input 
                                                            ...
                                                            className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none uppercase font-black focus:border-amber-500" 
                                                        />
                                                    </div>
                                                    ...
                                                    <div className="w-full lg:w-48 shrink-0">
                                                        ...
                                                        <div className="flex gap-1">
                                                            <input 
                                                                ...
                                                                placeholder="Kol." 
                                                                className="flex-1 min-w-0 p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none font-black focus:border-amber-500 text-center" 
                                                            />
                                                            <select 
                                                                value={faza.jm || 'kom'} 
                                                                onChange={e=>updateFazu(s.id, faza.id, 'jm', e.target.value)} 
                                                                className="w-16 shrink-0 p-3 bg-theme-panel rounded-xl text-[10px] text-theme-text border border-theme-border outline-none focus:border-amber-500 cursor-pointer"
                                                            >
                                                                <option value="kom">kom</option><option value="m3">m³</option><option value="m2">m²</option><option value="m1">m1</option>
                                                            </select>
                                                        </div>
                                                    ...
                                                    <div className="flex-[1.5] w-full min-w-[140px]">
                                                        ...
                                                        <input 
                                                            ...
                                                            className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-amber-500" 
                                                        />
                                                    </div>
                                                    ...
                                                    <div className="w-full lg:w-auto shrink-0 flex items-center justify-end mt-2 lg:mt-0">
                                                        <button 
                                                            ...
                                                            className="w-full lg:w-auto bg-red-900/30 text-red-500 p-3 rounded-xl font-black hover:bg-red-500 hover:text-theme-text transition-all opacity-100 lg:opacity-0 group-hover:opacity-100 border border-red-500/20"
                                                        >
                                                            ✕ Ukloni
                                                        </button>
                                                    </div>
                                                    ...
                                                    {dostupneOznake.length > 0 && (
                                                        <div className="w-full mt-2 bg-theme-panel p-3 rounded-xl border border-theme-border">
                                                                ...
                                                                {dostupneOznake.map(o => {
                                                                    ...
                                                                    return (
                                                                        <button 
                                                                            ...
                                                                            className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase transition-all border ${isSelected ? 'bg-amber-600 border-amber-400 text-theme-text shadow-[0_0_10px_rgba(217,119,6,0.4)] scale-105' : 'bg-theme-panel border-theme-border text-slate-400 hover:bg-slate-700 hover:text-theme-text hover:border-slate-500'}`}
                                                                        >
                                                                            {isSelected ? '✓ ' : '+ '} {o}
                                                                        </button>
                                                                    )
                                        ...
                                        <div className="flex flex-col md:flex-row justify-between items-center mt-2 border-t border-theme-border pt-4 gap-3">
                                            <button 
                                            ...
                                            <div className="flex gap-2 w-full md:w-auto">
                                                <button 
                                                    ...
                                                    className="flex-1 px-4 py-3 bg-theme-panel text-theme-text rounded-xl text-[10px] uppercase font-black hover:bg-slate-700 border border-slate-600 transition-all"
                                                >
                                                    Sačuvaj u Nalog
                                                </button>
                                                <button 
                                                    onClick={() => snimiPraviloKaoSablon(s.sifra, faze)} 
                                                    className="flex-1 px-4 py-3 bg-emerald-600 text-theme-text rounded-xl text-[10px] uppercase font-black hover:bg-emerald-500 shadow-lg shadow-emerald-600/30 transition-all border border-emerald-400"
                                                >
                                                    💾 Snimi kao Šablon
                                                </button>
                                            </div>
                                        ...
                                        {faze.length > 0 && faze[0].masina && (
                                            <div className="flex items-center gap-2 mt-2 bg-amber-500/10 p-3 rounded-xl border border-amber-500/20">
                                                <div className="w-2 h-2 rounded-box bg-amber-500 animate-pulse"></div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 max-h-[600px] overflow-y-auto animate-in slide-in-from-right">
                    ...
                    {nalozi.map(n => (
                        <div 
                            ...
                            className="bg-theme-card backdrop-blur-[var(--glass-blur)] border border-theme-border p-5 rounded-box shadow-lg cursor-pointer hover:border-purple-500 hover:-translate-y-1 transition-all relative overflow-hidden group"
                        >
                            ...
                            <div className="flex justify-between items-start border-b border-theme-border pb-3 mb-3 cursor-pointer">
                                <div>
                                    <p className="text-theme-accent text-base font-black flex items-center gap-2">
                                        {n.id}
                                    </p>
                                    <p className="text-theme-text text-xs font-bold mt-1">{n.kupac_naziv}</p>
                                </div>
                                <div className="text-right">
                                    <p className={`text-[9px] px-2 py-1 rounded font-black uppercase inline-block ${n.status === 'ZAVRŠENO' ? 'bg-emerald-900/30 text-emerald-400' : 'bg-purple-900/30 text-theme-accent'}`}>{n.status}</p>
                                    <p className="text-[9px] text-slate-500 uppercase mt-2">Rok: {formatirajDatum(n.rok_isporuke)}</p>
                                </div>
                            </div>
                            <div className="flex justify-between items-center mb-3">
                                <span className="text-[9px] text-slate-400 font-bold bg-theme-card px-2 py-1 rounded">Vezano: {n.broj_ponude || 'Nema'}</span>
                            ...
                            <div className="flex flex-wrap gap-2 pt-3 border-t border-theme-border">
                                {n.tip_naloga === 'FAZNI' ? (
                                    <>
                                        <button 
                                            onClick={(e) => { e.stopPropagation(); printRadniNalogSveFaze({ ...n, stavke_jsonb: n.stavke_jsonb, tehnologija_jsonb: n.tehnologija_jsonb }); }} 
                                            className="bg-amber-600 hover:bg-amber-500 text-theme-text px-3 py-1.5 rounded-lg text-[9px] font-black uppercase shadow-lg border border-amber-400 w-full mb-1"
                                        >
                                            🖨️ SVE FAZE (BOM KARTA)
                                        </button>
                                        {[...new Set(Object.values(n.tehnologija_jsonb || {}).flat().map(f => f.masina))].filter(Boolean).map(m => (
                                            <button 
                                                key={m} 
                                                onClick={(e) => { e.stopPropagation(); printRadniNalogZaMasinu({ ...n, stavke_jsonb: n.stavke_jsonb, tehnologija_jsonb: n.tehnologija_jsonb }, m); }} 
                                                className="flex-1 bg-theme-panel hover:bg-white hover:text-black text-slate-300 px-2 py-1.5 rounded-lg text-[8px] font-black uppercase shadow-md border border-slate-600 transition-colors"
                                            >
                                                🖨️ {m}
                                            </button>
                                        ))}
                                    ...
                                    <button 
                                        ...
                                        className="w-full bg-theme-panel hover:bg-white hover:text-black text-theme-text px-4 py-2 rounded-lg text-[9px] font-black uppercase shadow-md border border-slate-600 transition-colors"
                                    >
                                        🖨️ PRINTAJ NALOG
                                    </button>
                                )}
