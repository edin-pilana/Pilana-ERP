...
function DimBox({ label, val, set, disabled }) {
    return (
        <div className={`bg-slate-900 p-2 rounded-xl border border-slate-800 font-bold text-center ${disabled ? 'opacity-50' : ''}`}>
            <span className="text-[7px] text-slate-500 uppercase block mb-1 font-black">{label}</span>
            <input type="number" value={val} onChange={e => set(e.target.value)} disabled={disabled} className="w-full bg-transparent text-white font-black outline-none text-lg text-center" />
        </div>
    );
}
...
function PD_SearchableRN({ nalozi, value, onSelect, onScanClick }) {
    ...
    return (
        <div ref={wrapperRef} className="relative font-black w-full flex bg-[#0f172a] border border-blue-500/50 rounded-xl overflow-visible focus-within:border-blue-400 transition-all shadow-inner">
            <input value={search} onFocus={() => setOpen(true)} onKeyDown={handleKeyDown} onChange={e => { setSearch(e.target.value); setOpen(true); }} placeholder="Upiši broj naloga/ponude ili skeniraj..." className="flex-1 p-4 bg-transparent text-sm text-white outline-none uppercase font-black tracking-widest relative z-10 min-w-0" />
            <button onClick={onScanClick} className="px-6 bg-blue-600 text-white text-xl hover:bg-blue-500 transition-all border-l border-blue-500/50 relative z-10 shrink-0">📷</button>
            {open && search && (
                <div className="absolute top-full left-0 z-[100] w-full mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto text-left custom-scrollbar">
...
function PD_SmartSearchableInput({ value, onChange, list = [] }) {
    ...
    return (
        <div ref={wrapperRef} className="relative w-full h-full">
            <input value={search} onChange={e => { setSearch(e.target.value); setOpen(true); onChange(e.target.value); }} onFocus={() => setOpen(true)} onKeyDown={handleKeyDown} placeholder="Pronađi..." className="w-full h-full min-h-[45px] p-3 bg-transparent text-xs text-white outline-none font-black uppercase min-w-0" />
            {open && filtered.length > 0 && (
                <div className="absolute z-[100] w-full mt-1 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl max-h-60 overflow-y-auto text-left custom-scrollbar">
                    {filtered.map((item, index) => (
                        <div key={index} onClick={() => {onChange(item); setSearch(item); setOpen(false);}} onMouseEnter={() => setSelectedIndex(index)} className={`p-3 border-b border-slate-700 cursor-pointer text-xs font-black uppercase transition-colors ${index === selectedIndex ? 'bg-emerald-600 text-white' : 'text-slate-300 hover:bg-slate-700'}`}>
                            {item}
                        </div>
                    ))}
                </div>
...
export default function DoradaModule({ user, header, setHeader, onExit }) {
    ...
    const saas = useSaaS('dorada_modul', {
        ...
        polja_radnici: [
            { id: 'brentista', label: '👨‍🔧 ODGOVORNI RADNIK (ZA MAŠINOM)', span: 'col-span-1' },
            ...
            if (ucitaneStavke.length > 0) {
                ucitaneStavke.forEach(s => {
                    ...
                    mapiraneStavke.push({
                        ...
                        isFazni: isFazni,
                        dokument_url: katItem.dokument_url || null
                    });
                });
    ...
    const renderRadnikPolje = (polje) => {
        if (polje.id === 'brentista') return <div className="h-full w-full min-w-0"><PD_SmartSearchableInput value={brentista} onChange={handleBrentistaChange} list={radniciList} /></div>;
        if (polje.id === 'viljuskarista') return <div className="h-full w-full min-w-0"><PD_SmartSearchableInput value={viljuskarista} onChange={handleViljuskaristaChange} list={radniciList} /></div>;
        return null;
    };
        ...
        <div className="p-4 max-w-2xl mx-auto space-y-6 animate-in fade-in font-bold">
            ...
            <div className={`p-6 rounded-[2.5rem] shadow-2xl space-y-5 transition-all ${saas.isEditMode ? 'ring-2 ring-amber-500 border-dashed' : ''}`} style={{ backgroundColor: saas.ui.boja_kartice, borderColor: saas.isEditMode ? '' : 'rgba(245, 158, 11, 0.3)', borderWidth: saas.isEditMode ? '0' : '1px' }}>
                        ...
                        <div className="mt-4 bg-blue-900/10 border border-blue-500/20 p-4 rounded-2xl shadow-inner">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                {predlozeniPaketi.filter(p => !activeUlazIds.includes(p.paket_id)).map(p => (
                                    <button 
                                        key={p.id} 
                                        onClick={() => processUlaz(p.paket_id)} 
                                        className="bg-slate-800 text-slate-300 px-3 py-2 rounded-xl text-[10px] font-black uppercase hover:bg-blue-600 hover:text-white transition-all border border-slate-600 shadow-md flex items-center gap-2"
                                    >
                                        <span className="bg-slate-950 px-2 py-0.5 rounded text-white">{p.paket_id}</span>
                                        {p.naziv_proizvoda}
                                    </button>
                                ))}
                            </div>
                    ...
                    {rnStavke.length > 0 && (
                        <div className="mt-3 space-y-2 border-t border-blue-500/30 pt-3">
                            ...
                            {rnStavke.map(s => {
                                ...
                                }} className="flex flex-col md:flex-row justify-between items-start md:items-center p-3 bg-slate-800 rounded-xl cursor-pointer hover:bg-blue-600 transition-all border border-slate-700 shadow-md group">
                                    <div>
                                        ...
                                        {/* PREGLED PDF-a (AKO POSTOJI) */}
                                        {s.dokument_url && (
                                            <a href={s.dokument_url} target="_blank" onClick={e=>e.stopPropagation()} className="mt-2 inline-block bg-blue-600/30 text-blue-300 px-2 py-1 rounded text-[9px] uppercase font-black hover:bg-blue-600 hover:text-white transition-all border border-blue-500/50">📄 POGLEDAJ DOKUMENT</a>
                                        )}
                                    </div>
                ...
                <div className="pt-2 border-t border-slate-700">
                    <label className="text-[10px] text-amber-500 uppercase font-black ml-2 mb-2 block pt-2">📦 Započeti Izlazni Paketi na ovoj mašini:</label>
