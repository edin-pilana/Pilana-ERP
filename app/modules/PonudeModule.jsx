...
function PonudeSearchableProizvod({ katalog, value, onChange }) {
    ...
    const handleKeyDown = (e) => {
        ...
        else if (e.key === 'Enter') { e.preventDefault(); if (filtered.length > 0) { const k = filtered[selectedIndex]; const tekstZaPolje = `${k.sifra} | ${k.naziv} | Dim: ${k.visina}x${k.sirina}x${k.duzina}`; onChange(k.sifra, tekstZaPolje); setSearch(tekstZaPolje); setOpen(false); } } 
        else if (e.key === 'Escape') setOpen(false);
    };

    return (
        <div ref={wrapperRef} className="relative font-black w-full">
            <input value={search} onFocus={() => setOpen(true)} onKeyDown={handleKeyDown} onChange={e => { setSearch(e.target.value); setOpen(true); }} placeholder="Pronađi šifru ili naziv..." className="w-full p-3 bg-theme-panel rounded-xl text-xs text-theme-text border border-theme-border outline-none focus:border-blue-500" />
            {open && filtered.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-theme-panel border border-theme-border rounded-xl shadow-2xl max-h-60 overflow-y-auto">
                    {filtered.map((k, index) => {
                        ...
                        return (
                            <div key={k.sifra} onMouseEnter={()=>setSelectedIndex(index)} onClick={() => { onChange(k.sifra, tekstZaPolje); setSearch(tekstZaPolje); setOpen(false); }} className={`p-3 border-b border-theme-border cursor-pointer transition-all ${index === selectedIndex ? 'bg-theme-accent' : 'hover:bg-slate-700'}`}>
...
export default function PonudeModule({ onExit }) {
    ...
    const load = async () => {
        ...
        if (loggedUser.uloga === 'superadmin') { const {data: logs} = await supabase.from('sistem_audit_log').select('*').eq('akcija', 'RUČNA_IZMJENA_RABATA').order('vrijeme', { ascending: false }).limit(10); setSuperadminLogs(logs || []); }
    };
    ...
    const primijeniGlobalniRabat = async () => {
        const rabat = parseFloat(globalRabat); if (isNaN(rabat) || rabat < 0 || rabat > 100) return alert("Unesite validan procenat (0-100)!");
        ...
        setStavke(noveStavke); setGlobalRabat(''); await zapisiU_Log('RUČNA_IZMJENA_RABATA', `Globalni rabat ${rabat}% primijenjen na ponudi ${form.id}`); alert(`✅ Rabat od ${rabat}% je uspješno primijenjen na sve!`);
    };
    ...
    const snimiPonudu = async () => {
        if(!form.kupac_naziv) return alert("Kupac je obavezan!"); if(stavke.length === 0) return alert("Ponuda mora imati stavke!");
        const payload = {
            ...
            ukupno_bez_pdv: parseFloat(totals.osnovica), ukupno_rabat: parseFloat(totals.rabat), ukupno_sa_pdv: parseFloat(totals.za_naplatu), snimio_korisnik: loggedUser.ime_prezime, rn_modifikovan: form.rn_modifikovan
        };
        if (isEditingPonuda) {
            const { error } = await supabase.from('ponude').update(payload).eq('id', form.id);
            if(error) return alert("Greška: " + error.message); await zapisiU_Log('IZMJENA_PONUDE', `Ažurirana ponuda ${form.id}`); alert("✅ Ponuda uspješno ažurirana!");
        } else {
            const { error } = await supabase.from('ponude').insert([payload]);
            if(error) return alert("Greška pri snimanju: " + error.message); await zapisiU_Log('KREIRANA_PONUDA', `Ponuda ${form.id}`); alert("✅ Ponuda uspješno kreirana!");
        }
        if ((form.status === 'POTVRĐENA' || form.status === 'REALIZOVANA ✅') && parseFloat(form.depozit) > 0) {
            ...
            if (!postojiAvans) { await supabase.from('blagajna').insert([{ id: `KASA-${new Date().getFullYear()}-${Math.floor(Math.random() * 100000)}`, tip: 'ULAZ', kategorija: 'Avans po Ponudi', iznos: parseFloat(form.depozit), opis: `Automatski depozit: ${form.id}`, racun_id: form.id, datum: new Date().toISOString().split('T')[0], vrijeme_tekst: new Date().toLocaleTimeString('de-DE'), snimio_korisnik: loggedUser.ime_prezime || 'Sistem' }]); }
        }
        if (window.confirm("✅ Uspješno snimljeno! Da li želite isprintati PDF ove ponude?")) { kreirajPDF(); }
    ...
    const prihvatiIzmjene = async () => {
        ...
        setStavke(noveStavke); setForm({...form, rn_modifikovan: false}); await supabase.from('ponude').update({ rn_modifikovan: false, stavke_jsonb: noveStavke }).eq('id', form.id); await supabase.from('radni_nalozi').update({ modifikovan: false }).eq('id', povezaniRN.id); setPovezaniRN(null); alert("✅ Izmjene iz proizvodnje su PRIHVAĆENE!");
    };
    const odbijIzmjene = async () => {
        if(!povezaniRN) return; if(!window.confirm("Radni nalog će biti vraćen na originalne količine!")) return;
        setForm({...form, rn_modifikovan: false}); await supabase.from('ponude').update({ rn_modifikovan: false }).eq('id', form.id); await supabase.from('radni_nalozi').update({ stavke_jsonb: stavke, modifikovan: false }).eq('id', povezaniRN.id); setPovezaniRN(null); alert("❌ Izmjene su ODBIJENE!");
    };

    const kreirajPDF = () => {
        const stariNaslov = document.title; document.title = `ponuda_${form.id}_${(form.kupac_naziv || 'Nepoznat_Kupac').replace(/\s+/g, '_')}`;
        ...
        let redovi = stavke.map((s, i) => {
            let dimenzijeTekst = ''; if (s.dimenzije) dimenzijeTekst = `<span class="prod-dim">Dimenzije: ${s.dimenzije}</span>`; else if (s.debljina && s.sirina && s.duzina) dimenzijeTekst = `<span class="prod-dim">Dimenzije: ${s.debljina} x ${s.sirina} x ${s.duzina}</span>`;
        ...
        const htmlSadrzajTabela = `<style>table th, table td { padding: 4px 6px !important; } .footer { margin-top: 25px !important; font-size: 11px !important; display: flex; justify-content: space-between; } .napomena { width: 60%; color: #475569; line-height: 1.4; } .potpis { width: 30%; text-align: center; color: #64748b; }</style><div class="info-grid"><div class="info-col-left"><h4>Kupac / Klijent</h4><h2>${form.kupac_naziv}</h2><p>${odabraniKupac?.adresa || 'Adresa nije unesena'}</p><p style="margin-top: 3px;">PDV / ID: <b style="color: #0f172a;">${odabraniKupac?.pdv_broj || 'N/A'}</b></p></div><div class="info-col-right"><p>Paritet: <b>${form.paritet}</b></p><p style="margin-top: 2px;">Plaćanje: <b>${form.nacin_placanja}</b></p><p style="margin-top: 2px;">Valuta: <b>${form.valuta}</b></p><p style="color: #ec4899; margin-top: 8px;">Važi do: <b>${formatirajDatum(form.rok_vazenja)}</b></p></div></div><table><thead><tr><th style="width: 5%;">R.B.</th><th>Naziv Proizvoda i Šifra</th><th style="text-align:center;">Količina</th><th class="num">Cijena</th><th class="num">Rabat</th><th class="num">Ukupno (${form.valuta})</th></tr></thead><tbody>${redovi}</tbody></table><div class="footer"><div class="napomena"><b style="color: #0f172a; text-transform: uppercase;">Napomena uz ponudu:</b><br/>${form.napomena || 'Nema dodatnih napomena.'}</div><div class="potpis"><div style="border-bottom: 1px solid #cbd5e1; margin-bottom: 5px; height: 40px;"></div>Potpis ovlaštenog lica</div></div>`;

        const donjiTotaliHtml = `<table class="totals-table"><tr><td class="lbl">Iznos bez rabata:</td><td class="val">${totals.bez_rabata}</td></tr>${parseFloat(totals.rabat) > 0 ? `<tr><td class="lbl" style="color: #ec4899;">Uračunati rabat:</td><td class="val" style="color: #ec4899;">- ${totals.rabat}</td></tr>` : ''}${form.ukljuciPDV ? `<tr><td class="lbl">Osnovica za PDV:</td><td class="val">${totals.osnovica}</td></tr><tr><td class="lbl">PDV iznos (17%):</td><td class="val">${totals.pdv}</td></tr><tr><td class="lbl" style="padding-top: 6px; border-top: 1px solid #cbd5e1;">UKUPNO SA PDV:</td><td class="val" style="padding-top: 6px; border-top: 1px solid #cbd5e1;">${ukupnoSaPDV.toFixed(2)}</td></tr>` : `<tr><td class="lbl" style="padding-top: 6px; border-top: 1px solid #cbd5e1;">UKUPNO:</td><td class="val" style="padding-top: 6px; border-top: 1px solid #cbd5e1;">${ukupnoSaPDV.toFixed(2)}</td></tr>`}${depozit > 0 ? `<tr><td class="lbl" style="color: #10b981;">Uplaćen depozit:</td><td class="val" style="color: #10b981;">- ${depozit.toFixed(2)}</td></tr>` : ''}<tr class="grand-total"><td class="lbl">ZA NAPLATU:</td><td class="val">${preostaloZaNaplatu} ${form.valuta}</td></tr></table>`;
        printDokument('PONUDA', form.id, formatirajDatum(form.datum), htmlSadrzajTabela, '#ec4899', donjiTotaliHtml); setTimeout(() => { document.title = stariNaslov; }, 2000);
    };
        ...
        <div className="p-4 max-w-6xl mx-auto space-y-6 font-bold">
            ...
            {tab === 'nova' ? (
                <div className="space-y-4 animate-in slide-in-from-left max-w-4xl mx-auto">
                    ...
                    <div className={`p-6 rounded-box border-2 shadow-2xl space-y-4 transition-all relative z-[40] ${saas.isEditMode ? 'border-dashed border-amber-500 bg-black/20' : (isEditingPonuda ? 'border-amber-500/50 bg-theme-card backdrop-blur-[var(--glass-blur)]' : 'border-pink-500/30 bg-theme-card backdrop-blur-[var(--glass-blur)]')}`} >
                        ...
                        {trenutniProizvod && (
                            <div className="p-4 bg-blue-900/10 border border-blue-500/30 rounded-2xl animate-in zoom-in-95 space-y-4">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 items-end">
                                    ...
                                    <div className="col-span-2"><label className="text-[8px] text-slate-500 uppercase ml-2 block mb-1">Obračunava se po</label><div className="flex gap-1"><input type="number" value={stavkaForm.kolicina_obracun} readOnly className="flex-1 p-3 bg-blue-900/10 rounded-xl text-sm text-theme-accent font-black text-center border border-blue-500/20 outline-none cursor-not-allowed pointer-events-none opacity-80" /><select value={stavkaForm.jm_obracun} onChange={e=>setStavkaForm({...stavkaForm, jm_obracun:e.target.value})} className="w-20 p-3 bg-theme-panel rounded-xl text-xs text-theme-text outline-none border border-theme-border hover:border-blue-500 cursor-pointer transition-all"><option value="m3">m³</option><option value="m2">m²</option><option value="m1">m1</option><option value="kom">kom</option></select></div></div>
                                    <div className="col-span-2 md:col-span-5 grid grid-cols-2 gap-3 bg-black/20 p-3 rounded-xl border border-theme-border mt-2">
                                        <div>
